...
public class StudentGradeTrackerGUI extends JFrame {
    ...
    private RoundedPanel buildInputCard() {
        ...
        nameBlock.add(nameField);
        card.add(nameBlock);

        JPanel scoreBlock = new JPanel();
        scoreBlock.setOpaque(false);
        scoreBlock.setLayout(new BoxLayout(scoreBlock, BoxLayout.Y_AXIS));
        JLabel scoreLbl = styledLabel("SCORE (0-100)");
        scoreField = styledTextField(6);
        scoreBlock.add(scoreLbl);
        scoreBlock.add(Box.createVerticalStrut(4));
        scoreBlock.add(scoreField);
        card.add(scoreBlock);
    ...
    private JTextField styledTextField(int cols) {
        ...
        field.setPreferredSize(new Dimension(Math.max(pref.width, cols * 12), 32));
        return field;
    }
